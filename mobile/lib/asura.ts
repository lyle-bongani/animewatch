...
/** Fetches latest updated series from AsuraScans */
export async function getAsuraLatest(page = 1): Promise<AsuraSeriesCard[]> {
  try {
    ...
    while ((match = regex.exec(html)) !== null) {
      ...
      const ratingMatch = inner.match(
        /<span[^>]*class=[\"'][^\"']*(?:font-semibold|text-white)[^\"']*[\"'][^>]*>([0-9.]+)<\/span>/i
      );
...
export async function getAsuraChapterPages(
  ...
  try {
    const res = await fetch(
      `${BASE_URL}/comics/${encodeURIComponent(slug)}/chapter/${encodeURIComponent(chapter)}`,
      {
        headers: {
          "User-Agent":
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36",
    ...
    const seriesTitleMatch =
      html.match(
        /<a[^>]+href=[\"']\/comics\/[a-zA-Z0-9_-]+[\"'][^>]*>[\s\S]*?<h3[^>]*class=[\"'][^\"']*font-bold[^\"']*[\"'][^>]*>([^<]+)<\/h3>/i
      ) ||
    ...
    if (pages.length === 0) {
      const imgRegex =
        /src=[\"'](https?:\/\/cdn\.asurascans\.com\/asura-images\/chapters\/[^\"']+)[\"']/gi;
      let imgM;
